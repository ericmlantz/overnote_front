...
const { app, BrowserWindow, Tray, Menu, ipcMain, dialog, shell, systemPreferences } = require('electron') // Import Electron modules for app functionality
const fs = require('fs') // Import file system module
const { execSync } = require('child_process') // Import execSync for synchronous command execution
const path = require('path') // Import path module for handling file paths
const { getActiveAppContext } = require('./active-window') // Import function to get active application context
...
function startPolling() { 
  ...
  pollingInterval = setInterval(async () => { // Set an interval to poll every 500ms
    try {
      const context = await getCurrentContext() // Get the current context
      if (notesWindow && notesWindow.isFocused()) return // If notes window is focused, exit
      if (context !== previousContext) { // If context has changed
        previousContext = context // Update previous context
        console.log('Active app changed to:', context) // Log context change
        await updateNotesWindowTitle(context) // Update notes window title with new context
      }
    } catch (error) {
...
function stopPolling() { 
  if (pollingInterval) { // If polling is active
...
// IPC Setup
////////////
if (!ipcMain.eventNames().includes('get-current-context')) { // Check if 'get-current-context' event is already set
  ipcMain.handle('get-current-context', async () => { // Handle 'get-current-context' event
    try {
      const context = await getCurrentContext() // Get the current context
...
ipcMain.on('toggle-always-on-top', (event, isAlwaysOnTop) => {
  if (notesWindow) { // If notes window exists
...
async function getCurrentContext() {
  try {
    ...
    if (error.message === 'MAX_CONTEXT_FAILURES_REACHED') {
      console.warn('♻️ Restarting backend due to repeated context fetch failures...')
...
////////////////////////////////
async function updateNotesWindowTitle(context) { // Function to update the notes window title and context
  try {
    if (context === 'Overnote' && currentContext) { // If context is Overnote
      console.log('Context was Overnote; keeping current context:', currentContext); // Log current context
      context = currentContext; // Keep current context
    }

    if (context && context !== currentContext) { // If context is valid and different from current context
      currentContext = context; // Update current context
      if (notesWindow) { // If notes window exists
        notesWindow.setTitle(`${currentContext}`); // Set the window title
        notesWindow.webContents.send('update-context', currentContext); // Send updated context to renderer
      }
    } else {
...
/////////////////////////////////////////////////
async function setupContextListeners() { // Function to set up context listeners for notes window
  notesWindow.on('show', () => { // When notes window is shown
    startPolling() // Start polling for context updates
  })

  notesWindow.on('hide', () => { // When notes window is hidden
    stopPolling() // Stop polling for context updates
  })
...
// Create Notes Window
//////////////////////
function createNotesWindow() { // Function to create the notes window
  if (allNotesWindow && !allNotesWindow.isDestroyed()) { // If all notes window exists and is not destroyed
    allNotesWindow.close() // Close all notes window
    stopPolling() // Stop polling for context updates
  }

  if (notesWindow && !notesWindow.isDestroyed()) { // If notes window exists and is not destroyed
    notesWindow.hide() // Prevent visual flash of old data

    getCurrentContext() // Get current context
      .then((context) => { // On success
        console.log( // Log context refresh
          'Forcing full refresh of notes window for context:',
          context
        )
        currentContext = '' // Reset current context

        notesWindow.webContents.once('did-finish-load', () => { // When notes window finishes loading
          console.log( // Log updated context
  ...
  notesWindow = new BrowserWindow({ // Create notes window
    width: 472, // Set width
    height: 400, // Set height
    show: false, // Initially hidden
    alwaysOnTop: true, // Always on top
    webPreferences: { // Web preferences for the window
  ...
  notesWindow.on('close', (e) => { // Handle notes window close event
    if (!app.isQuitting) { // If not quitting the app
      e.preventDefault() // Prevent default close behavior
      notesWindow.hide() // Hide the window instead
    }
  })

  notesWindow.on('focus', () => { // Handle notes window focus event
...
////////////////////////////////
function toggleNotesWindow() { // Function to toggle notes window visibility
  ...
  const y = Math.round( // Calculate y position
    trayBounds.y + trayBounds.height / 2 - windowBounds.height / 2
  )

  notesWindow.setBounds({ // Set notes window bounds
    x: x,
    y: y,
  ...
  if (notesWindow.isVisible()) { // If notes window is visible
    notesWindow.hide() // Hide the window
  } else { // If notes window is hidden
    getCurrentContext() // Get current context
      .then((context) => { // On success
        ...
        setTimeout(() => { // Delay to send updated always on top state
          if (notesWindow && notesWindow.webContents) { // If notes window and web contents exist
            notesWindow.webContents.send('update-always-on-top', notesWindow.isAlwaysOnTop()); // Send updated always on top state
          }
        }, 100); // Delay for 100ms
      })
      .catch((error) => { // On error
...
//////////////////////////
function openAllNotesWindow() { // Function to create the All Notes window
  if (notesWindow && !notesWindow.isDestroyed()) { // If notes window exists and is not destroyed
  ...
  allNotesWindow = new BrowserWindow({ // Create all notes window
  ...
  allNotesWindow.on('closed', () => { // Handle all notes window closed event
    allNotesWindow = null // Reset all notes window variable
  })
...
app.on('ready', () => {
  ...
  const backendBinaryPath = isDev // Determine backend binary path based on dev or production
    ? path.join(__dirname, '../../overnote_back/dist/run_backend') // Development path
    : path.join(process.resourcesPath, 'run_backend'); // Production path
  
  backendProcess = spawn(backendBinaryPath, { // Spawn the backend process
    stdio: ['pipe', 'pipe', 'pipe'], // Set stdio to pipe
    env: { ...process.env, OVERNOTE_DB_PATH: sqlitePath } // Pass environment variables
  });
  
  backendProcess.stdout.on('data', (data) => { // Listen for data from backend stdout
    const output = data.toString(); // Convert data to string
    console.log(output); // Log output
    const match = output.match(/Starting development server at http:\/\/127\.0\.0\.1:(\d+)/); // Match backend port in output
  ...
  backendProcess.stderr.on('data', (data) => { // Listen for data from backend stderr
  ...
  ////////////////////////////////////////////////
  const iconPath = path.join( // Path to tray icon
    __dirname,
    '../public',
  ...
  tray.on('click', () => { // Handle tray icon click
    toggleNotesWindow() // Toggle notes window visibility
  })

  tray.on('right-click', () => { // Handle tray icon right-click
    const contextMenu = Menu.buildFromTemplate([ // Create context menu
      {
