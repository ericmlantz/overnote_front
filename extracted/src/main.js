...
function startPolling() {
  ...
  pollingInterval = setInterval(async () => {
    ...
    try {
      ...
      if (context !== previousContext || context.includes('Silhouette Studio')) {
        // If context has changed or is Silhouette Studio
...
async function getCurrentContext() {
  try {
    ...
    if (error.message === 'MAX_CONTEXT_FAILURES_REACHED') {
      console.warn(
        '♻️ Restarting backend due to repeated context fetch failures...'
      )
